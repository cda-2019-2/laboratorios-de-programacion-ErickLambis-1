...
sed 's/\b\([0-9]\)\//0\1\//' data.csv | sed 's/\([a-z]\)/\u\1/g' | sed 's/;;/;\\N;/g' | sed 's/;\r/;\\N/g' | sed 's/\\n/\\N/g' | sed 's/;N/;\\N/g' | sed 's/\/\([0-9]\)\([0-9]\);/\/20\1\2;/' | sed 's|/\([0-9]\)/|/0\1/|' | sed 's|\([0-9]*\)/\([0-9]*\)/\([0-9]*\);|\3-\2-\1;|' | sed -e 's|;;|\\N|g'| sed 's|;\([0-9]*\),|;\1.|g' | sed 's|;|,|g'
